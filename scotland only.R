...
selected_data <- selected_data %>% filter(str_detect(GEOGRAPHY_CODE, "S0")) %>% 
...
#add in Scot IMD data
selected_data <- merge(selected_data, simd, by.x = "GEOGRAPHY_CODE", by.y = "IZ2011_Code")
selected_data <- selected_data %>% rename("Index of Multiple Deprivation (IMD) Rank (where 1 is most deprived).x" = SIMD2020v2_Rank,
...
#merge 'em
selected_data <- merge(selected_data, msoanames, by.x = "GEOGRAPHY_CODE", by.y = "msoa11cd", all.x = T)
...
izbounds <- merge(izbounds, selected_data, by.x = "MSOA11CD", by.y = "GEOGRAPHY_CODE", all.x = T)
...
msoa.centroids_eandw <- merge(izcentroids, selected_data, by.x = "msoa11cd", by.y = "GEOGRAPHY_CODE", all.y = T) #merge in our data
...
labels2 <- sprintf("<strong>%s</strong><br/>%g COVID-19 AMR per 100k<sup></sup>",
                   izbounds$`Neighbourhood name`, round(izbounds$`COVID-19 Adjusted Mortality Rate per 100,000`,0)) %>% lapply(htmltools::HTML)
...
#page element title
title <- tags$div(HTML("COVID-19 deaths per 100,000 and deprivation,<br> March to July 2020, Scotland</br>"), 
                  style = "font-family: Open Sans;color: #2A2A2A;font-weight: bold; font-size: 22px; text-align: center"
)

#page element data sources
sources <- tags$div(HTML("Sources: Scottish Indices of Multiple Deprivation (2020), Deaths involving coronavirus (COVID-19) in Scotland, Week 28: Extra figures and tables, Scottish Government;<br>
                         Analysis: WPI Economics on behalf of CRC, (originally produced for Trust for London)"), 
                    style = "font-family: Open Sans;color: #2A2A2A;font-style: italic; font-size: 12px; text-align: left"
)

#map element
m2 <- leaflet(izbounds, height = "580px", options = list(padding = 100)) %>% setView(-3.5,57.8, 6.4) %>% 
...
addCircleMarkers(data = msoa.centroids_eandw_db, group = "circlegw",
  ...
  addLegend(pal = factpal, values = izbounds$`IMD MSOA Deciles`, 
            labels = levels(izbounds$`IMD MSOA Deciles`), position = "bottomright") %>% 
